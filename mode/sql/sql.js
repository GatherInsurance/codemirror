...
CodeMirror.defineMode("sql", function(config, parserConfig) {
  ...
  function tokenBase(stream, state) {
    ...
    if (support.hexNumber &&
      ...
      // placeholders
      return "variable-3";
    } else if (!stream.string.includes('{{') && !stream.string.includes('{{') && 
                (ch == "'" || (ch == '"' && support.doubleQuote))) {
  ...
  function hookParamIdentifier(stream) {
    console.log('sdfadfsasdfsdf')
    var ch;
    // while ((ch = stream.next()) != null) {
    //   if (ch == "\{" && !stream.eat("\}\}")) {
    //     console.log('asdlfkjaksdfkljasdlfjaklsdfajsdflkajsfljaksldfjalskdfjalsjdfkjlasdfjklsdf')
    //     return "variable-3";
    //   }
    // }
